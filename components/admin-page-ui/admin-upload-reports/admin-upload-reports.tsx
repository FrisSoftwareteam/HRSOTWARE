...
const UploadReports = ({ uuid }: Uuid) => {
  const [uploadedFiles, setUploadedFiles] = useState<File[]>([]);
  const [filesToUpload, setFilesToUpload] = useState<FileUploadProgress[]>([]);
  // const [controlNo, setControlNo] = useState("");

  // const { dataFiles, fetchFiles } = useFetchFile();
  ...
  const uploadCsvToAzure = async (
    formData: FormData,
    onUploadProgress: (progressEvent: AxiosProgressEvent) => void,
    cancelSource: CancelTokenSource
  ) => {
    console.log(FormData);
    const result = await axios.post(`/api/upload-report-chain`, formData, {
      onUploadProgress,
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    const fileUploadBatch = acceptedFiles.map((file: any) => {
  ...
  return (
    <>
      <div className="m-0">
        <label
          {...getRootProps()}
          className="relative flex flex-col items-center m-auto justify-center w-full py-6 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 "
        >
          <div className="text-center p-3 w-[500px]">
      ...
      {uploadedFiles.length > 0 && (
        <div>
          <p className="font-medium my-2 mt-6 mb-6 text-muted-foreground text-sm">
            Uploaded Files
          </p>
          <ScrollArea className="h-40">
            <div className="space-y-2 pr-3">
