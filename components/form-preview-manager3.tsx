...
export function FormPreviewManager3({ id }: any) {
  ...
  const onSubmit = async (values: z.infer<any>) => {
    const resultObject: any = {}; // Start with an empty object

    for (const key in resultObject) {
      // This loop won't execute because B has no keys
      if (values.hasOwnProperty(key)) {
        values[key] = resultObject[key];
      }
    }
      ...
      <>
        ...
        <Form {...form}>
          <form
            ...
            <div>
              Please keep in mind that the metrics listed below are graded based
              on the options that have been selected. As a result, you have the
              option of keeping options as is or to update them as needed.
            </div>
