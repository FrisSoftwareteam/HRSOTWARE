...
import { SortableFieldPreviewManager2 } from "./sortable-field-preview-manager2";
import { SortableFieldPreviewManager3 } from "./sortable-field-preview-manager3";
import { SortableFieldPreviewManager4 } from "./sortable-field-preview-manager4";
import { useGetAnswerId } from "@/zustand/store";

export function FormPreviewManager4({ id }: any) {
  // const { formstate, updateFormState } = useFormState();
  //const clearFormFields = useFormStore((state) => state.clearFormFields);
  const { data: data1, loadings, qustionId } = useGetAnswerId();
  ...
  //   queryKey: ["form_managers_data4"],
  //   queryFn: async () => {
  //     try {
  //       return await fetch(`/api/get-answer/${+data1}`)
  //         .then((res: any) => res.json())
  //         .then((data: any) => {
  //           console.log(data.data?.users_answered);

  //           // if (data?.data?.users_answered) {
  //           //   return data?.data?.users_answered?.question;
  //           // }
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryKey: ["formAnswer4", data1],

    queryFn: async () => {
      // The `enabled` option below ensures this function only runs when `id` is truthy.
      try {
        const response = await fetch(`/api/get-answer/${+qustionId}`, {
          cache: "no-store",
        });

        console.log(`Fetching form answer for ID`, qustionId);
  ...
  const onSubmit = async (values: z.infer<any>, e: any) => {
    e.preventDefault();
      ...
      <>
        ...
        <Form {...form}>
          <form
            ...
            className="mx-auto flex w-4/5 flex-col gap-6 p-4">
            ...
            <SortableContext
              ...
              <Button type="submit" className="w-full bg-lime-800">
                Save
              </Button>
            </SortableContext>
