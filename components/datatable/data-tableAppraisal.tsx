...
export function DataTableAppraisal<TData, TValue, TLoading>({
  columns,
  data,
  loading,
              ...
              <>
                <TableRow key={headerGroup.id} className="bg-slate-100">
                  {headerGroup.headers.map((header) => (
                    <TableHead
                      className="px-4 py-2 border border-slate-300 text-left font-bold "
                      key={header.id}
          ...
          <TableBody>
            {loading ? (
              Array.from({
                ...
                <TableRow key={index}>
                  {Array.from({
                    length: table.getHeaderGroups()[0].headers.length,
                  }).map((_, index) => (
                    <TableCell>
                      <Skeleton className="h-4 w-[100px] bg-yellow-600" />
                    </TableCell>
                  ))}
                </TableRow>
              ))
                ...
                <>
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell
                        className="border border-slate-300 px-4 py-2 p-2"
                        key={cell.id}
                      >
                        {flexRender(
